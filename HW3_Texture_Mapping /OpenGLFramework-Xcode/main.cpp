...
void RenderScene(int per_vertex_or_per_pixel) {	
    ...
    for (int i = 0; i < models[cur_idx].shapes.size(); i++)
    {
        ...
        offsets.push_back(Vector2(0.0, 0.0));   // 1
        offsets.push_back(Vector2(0.0, 0.75));  // 2
        offsets.push_back(Vector2(0.0, 0.5));   // 3
        offsets.push_back(Vector2(0.0, 0.25));  // 4
        
        offsets.push_back(Vector2(0.5, 0.0));   // 5
        offsets.push_back(Vector2(0.5, 0.75));  // 6
        offsets.push_back(Vector2(0.5, 0.5));   // 7
        
        glActiveTexture(GL_TEXTURE0);
        glUniform1i(iLocIsEyes, models[cur_idx].shapes[i].material.isEye);
        if(models[cur_idx].shapes[i].material.isEye){
            glUniform2f(iLocOffset, offsets[models[cur_idx].cur_eye_offset_idx].x, offsets[models[cur_idx].cur_eye_offset_idx].y);
            ...
            glUniform2f(iLocOffset, offsets[0].x, offsets[0].y);
            glBindTexture(GL_TEXTURE_2D, models[cur_idx].shapes[i].material.diffuseTexture);
        }
...
void LoadTexturedModels(string model_path)
{
    ...
    for (int i = 0; i < materials.size(); i++)
    {
        ...
        material.Kd = Vector4(materials[i].diffuse[0], materials[i].diffuse[1], materials[i].diffuse[2],  materials[i].diffuse[3]);
        material.Ks = Vector4(materials[i].specular[0], materials[i].specular[1], materials[i].specular[2], materials[i].specular[3]);
...
void setUniformVariables()
{
    ...
    iLocIsEyes = glGetUniformLocation(program, "isEye");
...
int main(int argc, char **argv)
{
    ...
    // create window
    GLFWwindow* window = glfwCreateWindow(WINDOW_WIDTH, WINDOW_HEIGHT, "000000000_HW3", NULL, NULL);
    if (window == NULL)
    {
    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        RenderScene(1);
        // render right view
        glViewport(screenWidth, 0, screenWidth, screenHeight *2);
        RenderScene(0);
